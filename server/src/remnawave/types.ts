...
export interface RemnaReader {
  ...
  /** Все юзеры панели, постранично внутри (список целиком в памяти — только для мока и тестов) */
  getAllUsers(): Promise<RemnaUser[]>;
  /**
   * Справочник постранично: страница ушла в onPage — и отпущена, целиком в памяти не держится.
   * На сотнях тысяч юзеров список целиком — сотни мегабайт и риск упереться в лимит кучи.
   * Возвращает число отданных юзеров.
   */
  streamUsers(onPage: (users: RemnaUser[]) => void | Promise<void>): Promise<number>;
