...
Коллектор опрашивает ноды через тот же API, что и «Обозреватель сессий» панели, но не по одной, а параллельно (панель 2.7 держит в работе 5 таких запросов, 3.x — 10), пропуская ноды без онлайн-юзеров, поэтому круг по сотням нод занимает десятки секунд. Наблюдения «пользователь → IP» складываются в SQLite. Панель он не изменяет — только читает. Ноды, отвалившиеся от панели, пропускаются до возвращения на связь; пользователи, удалённые из панели, удаляются и из локальной базы.
